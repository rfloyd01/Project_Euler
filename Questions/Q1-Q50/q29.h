...
std::pair<std::string, double> q29()
{
	...
	const int limit = 100; //maximum allowable exponent for problem
	...
	//higher exponent.
	std::vector<int> powerNumbers(baseTwoLimit + 1, 0);
	powerNumbers[1] = limit - 1; //can't raise 1 to any power so remove it

	for (int i = baseTwoLimit; i > 1; i--)
	{
		powerNumbers[i] = pow(limit, 1.0 / i) - 1; //remove 1 as an option
