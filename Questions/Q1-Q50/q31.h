...
std::pair<std::string, double> q31()
{
	...
	const int total_coins = 8, goal_value = 200;
	int coin_values[total_coins] = { 1, 2, 5, 10, 20, 50, 100, 200 }, total_ways[goal_value + 1] = { 0 };
	total_ways[0] = 1;

	for (int i = 0; i < total_coins; i++)
		for (int j = coin_values[i]; j <= goal_value; j++)
			total_ways[j] += total_ways[j - coin_values[i]];

	return { std::to_string(total_ways[200]), std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - run_time).count() / 1000000000.0 };
...
//add two 1p coins or a 2p coin... all the way on down to 1p. The key is to start with only the 1p coin. You can make 1p with a 1p
...
//up to the 200p coin. I feel like this description doesn't quite capture the essence of what's happening, maybe come through later
//and update this.
