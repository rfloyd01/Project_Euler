...
std::pair<std::string, double> q61()
{
	...
	for (int i = 10; i < 100; i++) //start at 10 because anything less would have a lead 0 and not be a 4-digit number
	{
...
number isn't used twice in the same chain (e.g. a number that is both triangular and hexagonal shouldn't appear twice in the same chain). My Euler code is faster so I kept it above, 
although, the HackerRank code is a little more complete as it works for multiple test cases. Here's the HackerRank solution:
--------
...
std::pair<std::string, double> q61()
{
	...
	int current_chain[chain_length] = { 0 }, useable_figurates = 0b111111000;
