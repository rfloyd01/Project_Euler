...
int_64x& int_64x::operator<<=(const unsigned int left_shift)
{
	...
	if (shift_amount == 0)
	{
		for (int i = start; i >= 0; i--) this->digits[i + new_words] = this->digits[i];

		//need to zero any left over words at the beginning
		for (int i = new_words - 1; i >= 0; i--) this->digits[i] = 0;
		return *this;
	}
...
//TYPECAST OPERATORS
int_64x::operator int()
{
	//Casting is done explicitly so that the compiler doesn't try to cast the int_64x type to a different type
	//for some of the built-in operators.
	int convert = 0;
	if (this->digits.size()) convert = this->digits[0];
	return convert;
}
int_64x::operator unsigned int()
{
	//Casting is done explicitly so that the compiler doesn't try to cast the int_64x type to a different type
	//for some of the built-in operators.
...
int_64x::operator unsigned long long()
{
	//Casting is done explicitly so that the compiler doesn't try to cast the int_64x type to a different type
	//for some of the built-in operators.
